...
export async function getParentCommitFromMaster(buildId: number, apis: Apis): Promise<CommitDetails> {
  ...
  if (build && build.triggerInfo) {
    ...
    console.log('The repository ID: ', repository);
    ...
    if (prId) {
      ...
      // const prDetails: GitInterfaces.GitPullRequest = await gitApi.getPullRequestById(prId, project);  //Need to be change
  ...
  async function getCommitParents(repoId: string, commitId: string, project: string): Promise<string> {
    ...
    const commitDetails = await octokit.git.getCommit({
      owner: 'sunilsurana',
      repo: 'fluentuifork1',
      commit_sha: commitId,
    });

    return commitDetails.data.sha;
  }

  // async function getCommitParentsGitHub(repoId: string, commitId: string, project: string): Promise<string[]> {
  //   const commitDetails: GitInterfaces.GitCommit = await gitApi.getCommit(commitId, repoId, project);
  //   console.log('Commit ID: ' + commitId);
  //   console.log(
  //     'Parents of the Commit: ' + commitDetails &&
